import { obtenerAlumno, agregarUnAlumno } from "./javascripts_mantenimiento_agenda_firestore.js";
...
function actualizarAgenda ()
{
  for (let i=1; i<=24; i++)
  {
    ...
    {    
      tema_agendado[dia_e-1][i-1]="0"
      duracion_agendada[dia_e-1][i-1]="0"
      mensaje_agendado[dia_e-1][i-1]= ""
      mail_agendados[dia_e-1][i-1]=" "
      disponibilidad[dia_e-1][i-1]=1
      let alfadia
      let alfames
      if (dia_e<10) {alfadia = ("0"+dia_e)} else { alfadia = dia_e}
      if (mes<10) {alfames = ("0"+mes)} else {alfames = mes}
      const fechayhora_a_desagendar = (anio+"-"+alfames+"-"+alfadia+"-"+i)
      desagendar_una_clase (fechayhora_a_desagendar)
    }
    else if (document.getElementsByName("check-"+i) [0].checked && mail_agendados[dia_e-1][i-1]!=mailadress) 
    {
      ...
      agendar_una_clase (fechayhora_a_agendar,duracion_agendada[dia_e-1][i-1],tema_agendado[dia_e-1][i-1], mail_agendados[dia_e-1][i-1], fechadelaagenda , i, mensaje_agendado[dia_e-1][i-1] )
     console.log ("agendar clase")
     console.log (dia_e)
     console.log (mes)
     console.log (anio)
...
async function crearCalendarioMensual (mes, anio)
{
  ...
  await consultarAgenda (aaaaymm)
    ...
    console.table (calendario_mensual)
...
function recuperarDatosAgendaMensual ()
{
